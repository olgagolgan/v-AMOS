...
class RelationalQueryProcessor(RelationalProcessor):
    ...
    # getVenuesByPublisherId: It returns a data frame with all the venues (i.e. the rows) that have been published by
    # the organisation having the identifier specified as input (e.g. "crossref:78").
    ...
    # getPublicationInVenue: It returns a data frame with all the publications (i.e. the rows) that have been
    # included in the venue having the identifier specified as input (e.g. "issn:0944-1344").
    ...
    def getJournalArticlesInIssue(self, issue, volume, journalId):
        with connect(self.dbPath) as con:
            ...
            WHERE  JournalArticles.issue = '{0}' AND JournalArticles.volume = '{1}' AND Venues_doi.venue_id = '{2}';""".format(
                issue, volume, journalId)
            df_sql = read_sql(query, con)
        return df_sql

    # getJournalArticlesInIssue: It returns a data frame with all the journal articles (i.e. the rows) that have been
    # included in the input issue (e.g. "9") of the input volume (e.g. "17") of the journal having the identifier
    # specified as input (e.g. "issn:2164-5515").
    ...
    # getJournalArticlesInJournal: It returns a data frame with all the journal articles (i.e. the rows) that have
    # been included, independently from the issue and the volume, in the journal having the identifier specified as
    # input (e.g. "issn:2164-5515").
    ...
    # "doi:10.1080/21645515.2021.1910000", "doi:10.3390/ijfs9030035" ]).
